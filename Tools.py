...
def norm_stack(mstack, val):
    cols_nor = []
    for col in range(0, mstack.shape[1]):
        col_std = np.divide((mstack[:, col] - val[col][0]), (val[col][1] - val[col][0]))
        cols_nor.append(col_std)
    mstack = np.array(cols_nor)
    return mstack.T
...
def RandomForestClassification(X_train,Y_train,X_test, Y_test, Ntree, Njobs, target_names):
    ...
    cl_Final = RandomForestClassifier(n_estimators=Ntree, n_jobs=Njobs, criterion= "mse")
...
def SVMclassification(X_train, Y_train, X_test, Y_test, target_names, vfolds):
    ...
    return cl_Final, OA, kappa, CM, CR, Y_pred, BestC, BestG

def SVMclassificationBest(X_train, Y_train, X_test, Y_test, target_names, mc, mg):
    cl_Final = SVC(kernel='rbf', gamma=mg, C=mc)
    cl_Final.fit(X_train, Y_train)
    print('Predicting!')
    Y_pred = cl_Final.predict(X_test)
    OA = accuracy_score(Y_test,Y_pred)
